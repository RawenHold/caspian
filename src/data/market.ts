export const marketCards = [
  {
    label: "Foreign arrivals, Jan-May 2026",
    value: "871.7K",
    detail: "Foreigners and stateless persons who arrived in Azerbaijan from 186 countries",
    source: "official",
    note: "State Statistical Committee / State Border Service: 11.0% less than Jan-May 2025; 70.0% arrived by air.",
  },
  {
    label: "Jan-May 2025 comparison",
    value: "~979K",
    detail: "Calculated from the official YoY comparison for Jan-May 2026",
    source: "calculated",
    note: "871.7K / 0.89; use as a period comparison, not a full-year 2025 figure.",
  },
  {
    label: "Last full-year baseline",
    value: "2.6267M",
    detail: "Foreigners and stateless persons who arrived in Azerbaijan in 2024",
    source: "official",
    note: "The official tourism XLS updated on 04.06.2026 still ends its annual country table at 2024.",
  },
  {
    label: "Hotel revenue, 2024",
    value: "694.2M AZN",
    detail: "Revenue of hotels and similar establishments",
    source: "official",
    note: "42.0% more than in 2023.",
  },
  {
    label: "Launch model 2026",
    value: "3.5M",
    detail: "Deck model for launch-window planning",
    source: "model",
    note: "Internal model assumption, not official tourism forecast.",
  },
  {
    label: "TAM · official inbound revenue",
    value: "$1.56B",
    detail: "Official inbound tourism revenue, 2024",
    source: "model",
    note: "Primary TAM layer; source should be cited as State Statistical Committee / Tourism Agency of Azerbaijan.",
  },
];

export const pmfMetrics = [
  "1 000+ заявок за 90 дней",
  "NPS >= 40",
  "Request -> confirmed service conversion 12-18%",
  "50+ проверенных партнёров",
  "Support cases per confirmed request",
  "CTA demand by informational section",
];

export const marketSources = [
  "Official: State Statistical Committee of Azerbaijan, 19.06.2026 - 871.7K foreigners/stateless persons arrived in Jan-May 2026 from 186 countries, 11.0% less than Jan-May 2025.",
  "Calculated from official YoY comparison: Jan-May 2025 comparable arrivals were approximately 979K; this is not a full-year 2025 figure.",
  "Official: State Statistical Committee tourism XLS updated 04.06.2026 - annual country table currently ends at 2.6267M arrivals in 2024.",
  "Official: State Statistical Committee of Azerbaijan, Activities of hotels and similar establishments in 2024, 27.01.2025 - 4.2828M hotel nights and 694.2M AZN hotel revenue.",
  "Official: COP29 was held in Baku in November 2024; the site uses it as visibility context, not as quantified tourism uplift.",
  "Model: SAM uses 25-45% online penetration benchmark, capture rate target is 0.5%, SOM Y5 is $3.5M net revenue.",
  "Provider/legal: PSP, mobile wallet and Technopark benefits are roadmap inputs and require provider confirmation, contracts and legal review.",
];
